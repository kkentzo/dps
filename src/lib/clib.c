...
// returns -1 if something wrong happened, 0 if all was OK
int copy_file(const char *source, const char *dest) {

  int source_fd = open(source, O_RDONLY);
  int dest_fd = open(dest, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
  ...
  while (1) {
    ssize_t result = read(source_fd, &buf[0], sizeof(buf));
    if (! result) 
      break; // finished
    ...
    err = write(dest_fd, &buf[0], result);
    if (err != result)
      break;
  }
...
// create a unique file in DIR (which can be NULL) based on TEMPLATE
// TEMPLATE should end with "XXXXXX" and should be a char []
// (because mkstemp() will replace the Xs with a unique identifier)
// returns a GString with the filename if all's OK, otherwise NULL
GString *mk_unique_file(char *template, const char *dir) {

  GString *fname = g_string_new(dir);

  g_string_append_path(fname, template);

  int fd = mkstemp(fname->str);
  if (fd < 0) {
    // error -- free the string
    g_string_free(fname, TRUE);
    fname = NULL;
  } else {
